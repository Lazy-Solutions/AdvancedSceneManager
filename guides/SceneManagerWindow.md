...
Play buttons enters play mode and starts ASM startup sequence (opens startup collections) as if a build was started.
...
> View patches
>
> Views currently available patches on github.
...
> Reset
>
> Resets Advanced Scene Manager by deleting [profiles](Profile.md), [collections](SceneCollection.md), and [ASM Scenes](Scene.md).
...
>
> The custom [splash screen](SplashScreen.md) to display during startup, plays after Unity's splash screens, if enabled.
>
> Provides more flexibility than Unity's splash screen since ours is merely implemented as scenes with a script using IEnumerator callback.
...
>
> ASM has a [default pause screen](PauseScreenUtility.md) for those times when you build and then forget that no one has created a pause screen yet, forcing you to alt-f4, then restart again.
>
> This toggles default pause screen on or off.
...
>
> Checking toggle means first option is used.
>
...
The plugin manager can be used to install plugins for ASM.

It also contains links to samples on github.

![](https://github.com/Lazy-Solutions/AdvancedSceneManager/raw/ma../image/plugins-and-samples.PNG?raw=true)

#### Auto plugins

Some plugins will be automatically enabled or disabled depending on whatever its dependency is installed or not.

Dependencies will also be installed / uninstalled when plugin is enabled or disabled in plugin manager.

#### Available plugins

> [plugin.asm.netcode](https://github.com/Lazy-Solutions/plugin.asm.netcode)
>
...
Note that compilation errors may occur when uninstalling a dependency in package manager, when this happens you can either:

> Disable plugin from plugin manager

> Disable [pragma / scripting define](https://docs.unity3d.com/Manual/CustomScriptingSymbols.html) in player settings
