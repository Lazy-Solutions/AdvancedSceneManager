SceneAction is an action that is to be performed by the scene manager, such as opening a [scene](Scene.md). Used in [SceneOperation](SceneOperation.md).

### Default actions
##### Actions used by SceneOperation

> SceneLoadAction\
Loads a [scene](Scene.md), but does not activate it. This is the same as preload.

> SceneFinishLoadAction\
Finishes loading a [scene](Scene.md), calls [Start()](https://docs.unity3d.com/ScriptReference/MonoBehaviour.Start.html) and [Awake()](https://docs.unity3d.com/ScriptReference/MonoBehaviour.Awake.html).

> SceneUnloadAction\
Unloads a [scene](Scene.md).

> SceneOpenCallbackAction\
Calls all [ISceneOpen](Callbacks.md) callbacks on a scene.

> SceneCloseCallbackAction\
Calls all [ISceneClose](Callbacks.md) callbacks on a scene.

> OpenAndRunCallbackAction<T>\
Opens the [scene](Scene.md) and finds the first script instance that implements T and runs a callback defined on T.\
Closes [scene](Scene.md) if not found.\
Makes use of SceneOpenAction to open scene.

> RunCallbackAndCloseAction<T>\
Runs a callback on T and optionally closes the [scene](Scene.md).\
...
> StartupAction\
Represents the startup process in ASM. [AggragateAction](SceneAction.md#other-actions), executes the following actions:
...
> QuitAction\
Runs callbacks registered through [SceneManager.runtime.RegisterQuitCallback()](SceneManagerWindow.md#runtime), and then quits the game. Can be cancelled through [SceneManager.runtime.CancelQuit()](SceneManagerWindow.md#runtime).
...
Represents an action that can be overriden. SceneLoadAction, SceneFinishLoadAction, SceneUnloadAction, are overridable in order to support the [addressables support package](SupportPackages.md). Please take great care when overriding default ASM actions in user code as this may cause issues.
...
Should be called in DoAction when done. If a [scene](Scene.md) was opened, you may want to pass the OpenSceneInfo.
...
SceneOpenAction: The [scene](Scene.md) that was opened.\
SceneCloseAction: The [scene](Scene.md) that is about to be closed, is set to null when done.

> [Scene](Scene.md) scene { get; protected set; }\
The [scene](Scene.md) to perform the action on. Null if no value set in constructor.

> [SceneCollection](SceneCollection.md) collection { get; protected set; }\
The [collection](SceneCollection.md) that is being opened.

> bool isDone { get; protected set; }\
Is this scene action done? Use Done() to set this to true.

> float progress { get; protected set; }\
The progress of this [scene](Scene.md) action.
